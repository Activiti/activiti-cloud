...
public abstract class ModelingContextSteps<M> {
    ...
    protected EntityModel<M> checkAndGetCurrentContext(
        Class<M> expectedCurrentContextClass
    ) {
    ...
    public void openModelingObject(ModelingIdentifier<M> identifier) {
        Optional<EntityModel<M>> currentModelingObject = getAvailableModelingObjects()
            .stream()
            .filter(modelingObject ->
                identifier.test(modelingObject.getContent())
            )
            .findFirst()
            .map(resource -> resource.getLink(SELF).get())
            .map(Link::getHref)
        ...
        modelingContextHandler.setCurrentModelingObject(
            currentModelingObject.get()
        );
    }
    ...
    protected boolean existsInCollection(
        ModelingIdentifier<M> identifier,
    ...
    protected EntityModel<M> dirty(EntityModel<M> resource) {
        dirtyContextHandler.dirty(
            modelingUri(resource.getLink(SELF).get().getHref())
        );
        return resource;
    }
    ...
    protected FileContent toFileContent(Response response) throws IOException {
        String contentType = Optional
            ...
            .orElseThrow(() ->
                new RuntimeException("No Content-Type header in feign response")
            );
        ...
        String filename = Optional
                ...
                contentTypes
                    ...
                    .map(contentDisposition ->
                        contentDisposition.substring(
                            HEADER_ATTACHEMNT_FILENAME.length()
                        )
                    )
                    .orElse(null)
            )
            .orElseThrow(() ->
                new RuntimeException(
                    "No Content-Disposition header in feign response"
                )
            );
