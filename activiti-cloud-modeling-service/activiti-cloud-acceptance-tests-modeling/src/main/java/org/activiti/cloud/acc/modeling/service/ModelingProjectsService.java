...
public interface ModelingProjectsService
        extends FeignRestDataClient<ModelingProjectsService, Project> {
    ...
    default Response exportProjectByUri(String uri) {
        return FeignRestDataClient.builder(
                        new FormEncoder(new JacksonEncoder()), new Decoder.Default())
                .target(getType(), uri)
                .exportProject();
    }
    ...
    default EntityModel<Model> importProjectModelByUri(String uri, File file) {
        return FeignRestDataClient.builder(new FormEncoder(new JacksonEncoder()), modelingDecoder)
                .target(getType(), uri)
    ...
    static ModelingProjectsService build(Encoder encoder, Decoder decoder, String baseUrl) {
        return FeignRestDataClient.builder(encoder, decoder)
                .target(ModelingProjectsService.class, baseUrl + PATH);
    }
}
