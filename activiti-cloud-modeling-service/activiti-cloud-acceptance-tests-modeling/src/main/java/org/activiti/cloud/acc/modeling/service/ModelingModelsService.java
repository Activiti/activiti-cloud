 ...
 */
public interface ModelingModelsService
    extends FeignRestDataClient<ModelingModelsService, Model> {
    String PATH = "/v1/models";

    static ModelingModelsService build(
        Encoder encoder,
        Decoder decoder,
        String baseUrl
    ) {
    ...
    default Response validateModelByUri(String uri, FormData file) {
        return FeignRestDataClient
            .builder(new FormEncoder(encoder()), modelingDecoder)
            .target(getType(), uri)
            .validateModel(file);
    }
}
