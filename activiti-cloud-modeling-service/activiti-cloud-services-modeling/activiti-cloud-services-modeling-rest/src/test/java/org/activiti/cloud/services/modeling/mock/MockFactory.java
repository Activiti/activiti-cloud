...
public class MockFactory {
    ...
    public static ModelEntity processModelWithExtensions(
            String name, Map<String, Extensions> extensions) {
        return processModelWithExtensions(
                null, name, extensions != null ? extensions.get(name) : null);
    }

    public static ModelEntity processModelWithExtensions(
            String name, Extensions extensions, byte[] content) {
        return processModelWithExtensions(null, name, extensions, content);
    }

    public static ModelEntity processModelWithExtensions(
            ProjectEntity parentProject, String name, Extensions extensions) {
        return processModelWithExtensions(parentProject, name, extensions, null);
    }

    public static ModelEntity processModelWithExtensions(
            ProjectEntity parentProject, String name, Extensions extensions, byte[] content) {
    ...
    public static ModelEntity processModelWithContent(
            ProjectEntity project, String name, byte[] content) {
        return processModelWithContent(project, name, new String(content));
    }

    public static ModelEntity processModelWithContent(
            ProjectEntity project, String name, String content) {
        return processModelWithContent(project, name, null, content);
    }

    public static ModelEntity processModelWithContent(
            ProjectEntity project, String name, Extensions extensions, byte[] content) {
        return processModelWithContent(project, name, extensions, new String(content));
    }

    public static ModelEntity processModelWithContent(
            ProjectEntity project, String name, Extensions extensions, String content) {
    ...
    public static Map<String, Extensions> extensions(byte[] bytes) {
        try {
            return getExtensionMapFromJson(
                    new ObjectMapper().readValue(bytes, ModelEntity.class).getExtensions());
        } catch (IOException ex) {
            throw new RuntimeException(ex);
        }
    }

    private static Map<String, Extensions> getExtensionMapFromJson(Map<String, Object> map)
            throws IOException {
    ...
    public static Extensions extensions(
        ...
        extensions.setVariablesMappings(
                singletonMap(
                        serviceTask, Map.of(INPUTS, inputsMappings, OUTPUTS, outputsMappings)));
        return extensions;
    }
    ...
    public static Map<String, ProcessVariableMapping> inputsMappings(String... processVariables) {
        return Arrays.stream(processVariables)
                .collect(
                        Collectors.toMap(Function.identity(), MockFactory::processVariableMapping));
    }
    ...
    public static ModelEntity connectorModel(
            ProjectEntity parentProject, String name, byte[] content) {
    ...
    public static FileContent processFileContentWithCallActivity(
            String mainProcessName, Model callActivity, byte[] content) {
    ...
    public static MockMultipartFile multipartExtensionsFile(Model model, byte[] content) {
        return new MockMultipartFile(
                ...
                CONTENT_TYPE_JSON,
                new String(content)
                        .replaceAll("\"id\": \".*\"", "\"id\": \"process-" + model.getId() + "\"")
                        .getBytes());
    }

    public static MockMultipartFile multipartProcessFile(Model model, byte[] content) {
        return new MockMultipartFile(
                "file", model.getName() + "." + BPMN20_XML, CONTENT_TYPE_XML, content);
    }
}
