...
public class ModelController implements ModelRestApi {
    ...
    public EntityModel<Model> createModel(
            @PathVariable String projectId, @Valid @RequestBody Model model) {
        Project project = projectController.findProjectById(projectId);
    ...
    public EntityModel<Model> updateModel(
            @PathVariable String modelId, @Valid @RequestBody Model model) {
    ...
    public void updateModelContent(
            @PathVariable String modelId, @RequestPart(UPLOAD_FILE_PARAM_NAME) MultipartFile file)
            throws IOException {
    ...
    @Override
    public void getModelContent(HttpServletResponse response, @PathVariable String modelId)
            throws IOException {
    ...
    @Override
    public void getModelDiagram(HttpServletResponse response, @PathVariable String modelId)
            throws IOException {
        Model model = findModelById(modelId);
                        ...
                        .orElseThrow(
                                () ->
                                        new NotAcceptableStatusException(
                                                "Model content cannot be retrieved as svg image: "
                                                        + modelId));
    ...
    public EntityModel<Model> importModel(
        ...
        return representationModelAssembler.toModel(
                modelService.importSingleModel(
                        project, findModelType(type), multipartToFileContent(file)));
    }
    ...
    public PagedModel<EntityModel<ModelType>> getModelTypes(Pageable pageable) {
        return pagedModelTypeAssembler.toModel(
                pageable, modelTypeService.getModelTypeNames(pageable), modelTypeAssembler);
    }
    ...
    public void validateModel(
        ...
        if (StringUtils.isEmpty(projectId)) {
            modelService.validateModelContent(
                    findModelById(modelId), multipartToFileContent(file), validateUsage);
        } else {
            Project project = projectController.findProjectById(projectId);
            modelService.validateModelContent(
                    findModelById(modelId), multipartToFileContent(file), project, validateUsage);
        }
    }
    ...
    public void validateModelExtensions(
            ...
            @Parameter(description = VALIDATE_EXTENSIONS_FILE_PARAM_DESCR)
                    @RequestPart(UPLOAD_FILE_PARAM_NAME)
                    MultipartFile file,
            @RequestParam(value = PROJECT_ID_PARAM_NAME, required = false) String projectId)
            throws IOException {
        ...
        if (StringUtils.isEmpty(projectId)) {
            modelService.validateModelExtensions(
                    findModelById(modelId), multipartToFileContent(file));
        } else {
            Project project = projectController.findProjectById(projectId);
            modelService.validateModelExtensions(
                    findModelById(modelId), multipartToFileContent(file), project);
        }
    }
    ...
    public EntityModel<Model> putProjectModelRelationship(
            ...
            @RequestParam(value = FORCE_PARAM_NAME, required = false, defaultValue = "false")
                    boolean force) {
        ...
        if (force
                && (ModelScope.PROJECT.name().equals(scope)
                        || (scope == null
                                && modelToBeUpdated.getScope().equals(ModelScope.PROJECT)))) {
        ...
        return representationModelAssembler.toModel(
                modelService.updateModel(modelToBeUpdated, modelToBeUpdated));
    }

    @Override
    public EntityModel<Model> deleteProjectModelRelationship(
            @PathVariable String projectId, @PathVariable String modelId) {
    ...
    private boolean hasSameModelName(Model storedModel, Model modelToBeUpdated) {
        return storedModel.getType().equals(modelToBeUpdated.getType())
