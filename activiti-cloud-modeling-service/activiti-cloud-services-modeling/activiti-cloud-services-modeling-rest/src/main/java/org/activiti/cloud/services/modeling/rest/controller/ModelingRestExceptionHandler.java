...
public class ModelingRestExceptionHandler {
    ...
    private static final Logger logger =
            LoggerFactory.getLogger(ModelingRestExceptionHandler.class);
    ...
    public ErrorAttributes errorAttributes() {
        return new DefaultErrorAttributes() {
            ...
            public Map<String, Object> getErrorAttributes(
                ...
                Stream<ModelValidationError> bindingErrors =
                        Optional.ofNullable((List<ObjectError>) errorAttributes.get("errors"))
                                .map(this::transformBindingErrors)
                                .orElse(Stream.empty());
                Stream<ModelValidationError> semanticErrors =
                        resolveSemanticErrors(webRequest, errorAttributes);
                Stream<ModelValidationError> modelValidationErrorStream =
                        Stream.concat(bindingErrors, semanticErrors);
                List<ModelValidationError> collectedErrors =
                        modelValidationErrorStream.collect(Collectors.toList());
            ...
            private Stream<ModelValidationError> resolveSemanticErrors(
                    WebRequest webRequest, Map<String, Object> errorAttributes) {
    ...
    public void handleBadRequestException(Exception ex, HttpServletResponse response)
            throws IOException {
    ...
    public void handleDataIntegrityViolationException(
            DataIntegrityViolationException ex, HttpServletResponse response) throws IOException {
    ...
    public void handleModelNameConflictException(
            ModelNameConflictException ex, HttpServletResponse response) throws IOException {
        logger.error(ex.getMessage(), ex);
        response.sendError(CONFLICT.value(), ex.getMessage());
    }

    @ExceptionHandler(ModelScopeIntegrityException.class)
    public void handleModelScopeIntegrityException(
            ModelScopeIntegrityException ex, HttpServletResponse response) throws IOException {
        logger.error(ex.getMessage(), ex);
        response.sendError(CONFLICT.value(), ex.getMessage());
    }

    @ExceptionHandler({DataAccessException.class, PersistenceException.class, SQLException.class})
    public void handleDataAccessException(Exception ex, HttpServletResponse response)
            throws IOException {
