...
public abstract class JsonSchemaModelValidator implements ModelValidator {
    ...
    private final Logger log = LoggerFactory.getLogger(
        JsonSchemaModelValidator.class
    );
    ...
    public void validate(byte[] bytes, ValidationContext validationContext) {
        ...
        try {
            ...
            processExtensionJson =
                new JSONObject(new JSONTokener(new String(bytes)));
            schemaLoader().load().build().validate(processExtensionJson);
        } catch (JSONException jsonException) {
            log.debug(
                "Syntactic model JSON validation errors encountered",
                jsonException
            );
    ...
    private List<ModelValidationError> getValidationErrors(
        ...
        return getValidationExceptions(validationException)
            .map(exception ->
                toModelValidationError(exception, prcessExtenstionJson)
            )
            .distinct()
            .collect(Collectors.toList());
    }

    private Stream<ValidationException> getValidationExceptions(
        ValidationException validationException
    ) {
        return Optional
            ...
            .map(exceptions ->
                exceptions.stream().flatMap(this::getValidationExceptions)
            )
    ...
    private ModelValidationError toModelValidationError(
        ...
        if (
            unProcessedProperties != null &&
            unProcessedProperties.get("message") != null
        ) {
            HashMap<String, String> errorMessages = (HashMap<String, String>) unProcessedProperties.get(
                "message"
            );
            String errorMessage = errorMessages.get(
    ...
    private String resolveExpression(
        ...
        if (message.contains("{{name}}")) {
            ...
            message =
                message.replace(
                    "{{name}}",
                    getValueFromJson(path, prcessExtenstionJson)
                );
