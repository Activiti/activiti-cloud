 ...
 */
public class ProcessExtensionMessageMappingValidator
        ...
        if (flowNode != null) {
            errorStream =
                extensionMapping
                    ...
                    .filter(mappingModel ->
                        mappingModel.getAction() == ServiceTaskActionType.INPUTS
                    )
                    .map(this::validate)
    ...
    private boolean isMessageElement(Event element) {
        if (
            element.getEventDefinitions() != null &&
            element.getEventDefinitions().size() > 0
        ) {
            return (
                element
                    .getEventDefinitions()
                    .stream()
                    .filter(definition ->
                        definition instanceof MessageEventDefinition
                    )
                    .toArray()
                    .length >
                0
