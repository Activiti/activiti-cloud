...
public class ProcessExtensionsTaskMappingsValidator implements ProcessExtensionsValidator {
    ...
    public static final String UNKNOWN_TASK_VALIDATION_ERROR_PROBLEM =
            "Unknown task in process extensions: %s";
    public static final String UNKNOWN_TASK_VALIDATION_ERROR_DESCRIPTION =
    ...
    public ProcessExtensionsTaskMappingsValidator(
    ...
    public Stream<ModelValidationError> validateExtensions(
        ...
        return extensions.getVariablesMappings().entrySet().stream()
                .flatMap(
                        taskMapping ->
                                validateTaskMapping(
                                        bpmnModel.getId(),
    ...
    private Map<String, Constant> getTaskConstants(Extensions extensions, String taskKey) {
                ...
                .map(
                        task ->
                                validateTaskMappings(
                                        processId, task, extensionMapping, taskConstants, context))
                .orElseGet(
                        () ->
                                Stream.of(
                                        new ModelValidationError(
                                                format(
                                                        UNKNOWN_TASK_VALIDATION_ERROR_PROBLEM,
                                                        taskId),
                                                format(
                                                        UNKNOWN_TASK_VALIDATION_ERROR_DESCRIPTION,
                                                        processId,
                                                        taskId))));
    }
    ...
    private Stream<ModelValidationError> validateTaskMappings(
        ...
        List<MappingModel> taskMappings = toTaskMappings(processId, task, taskMappingsMap);
        return taskMappingsValidators.stream()
                .flatMap(
                        validator ->
                                validator.validateTaskMappings(
                                        taskMappings, taskConstants, validationContext));
    }

    private List<MappingModel> toTaskMappings(
            String processId,
            FlowNode task,
            Map<ServiceTaskActionType, Map<String, ProcessVariableMapping>> taskMappingsMap) {
        return taskMappingsMap.entrySet().stream()
                .map(
                        taskMappingEntry ->
                                new MappingModel(
                                        processId,
