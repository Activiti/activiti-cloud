...
public class ModelServiceImpl implements ModelService {
    ...
    public void validateModelContent(Model model, ValidationContext validationContext) {
        validateModelContent(model.getType(), modelRepository.getModelContent(model), validationContext);
    }

    @Override
    public void validateModelContent(Model model, Project project) {
        validateModelContent(model.getType(), modelRepository.getModelContent(model), createValidationContext(project));
    }
    ...
    private void validateModelContentAndUsage(Model model, byte[] modelContent, ValidationContext validationContext) {
        emptyIfNull(modelContentService.findModelValidators(model.getType()))
            .stream()
            .forEach(modelValidator -> modelValidator.validateModelContent(model, modelContent, validationContext, true)
            );
    }

    private void validateModelContent(String modelType, byte[] modelContent, ValidationContext validationContext) {
