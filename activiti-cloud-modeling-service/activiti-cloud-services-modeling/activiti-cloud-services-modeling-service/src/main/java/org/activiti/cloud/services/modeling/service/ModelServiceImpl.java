...
/**
 * Business logic related to {@link Model} entities including process models, form models,
 * connectors, data models and decision table models.
 */
...
public class ModelServiceImpl implements ModelService {
    ...
    public List<Model> getAllModels(Project project) {
        return modelTypeService.getAvailableModelTypes().stream()
    ...
    private void checkIfModelNameExistsInProject(Project project, Model model) {
        Optional<Model> existingModel =
                modelRepository.findModelByNameInProject(project, model.getName(), model.getType());
    ...
    public Model updateModel(Model modelToBeUpdated, Model newModel) {
        if (newModel.hasProjects()) {
            newModel.getProjects().stream()
                    .forEach(
                            project ->
                                    checkIfModelNameExistsInProject((Project) project, newModel));
        }
        checkModelScopeIntegrity(newModel);

        findModelUpdateListeners(modelToBeUpdated.getType()).stream()
    ...
    public Model copyModel(Model modelToBeCopied, Project project) {
        ...
        modelIdentifiers.put(
                String.join(
                        MODEL_IDENTIFIER_SEPARATOR,
    ...
    @Override
    public Model updateModelContent(Model modelToBeUpdated, FileContent fileContent) {

        FileContent fixedFileContent =
                modelIdentifiers.isEmpty()
                        ? fileContent
                        : overrideModelContentId(modelToBeUpdated, fileContent);
        ...
        if (modelToBeUpdated.getType().equals(PROCESS)
            ...
            modelToBeUpdated.setCategory(
                    safeGetBpmnModel(fixedFileContent.getFileContent()).getTargetNamespace());
        }
        ...
        try {
            Optional.ofNullable(modelToBeUpdated.getType())
                    ...
                    .flatMap(
                            validator ->
                                    validator.convertToModelContent(
                                            fixedFileContent.getFileContent()))
                    .ifPresent(
                            modelContent ->
                                    modelToBeUpdated.setTemplate(modelContent.getTemplate()));
        ...
        return modelRepository.updateModelContent(modelToBeUpdated, fixedFileContent);
    }
    ...
    public FileContent overrideModelContentId(Model model, FileContent fileContent) {
        return modelContentService
                ...
                .map(
                        modelContentConverter ->
                                modelContentConverter.overrideModelId(
                                        fileContent, modelIdentifiers))
                .orElse(fileContent);
    }

    @Override
    public Optional<ModelContent> createModelContentFromModel(
            Model model, FileContent fileContent) {
        return (Optional<ModelContent>)
                modelContentService
                        .findModelContentConverter(model.getType())
                        .map(
                                modelContentConverter ->
                                        modelContentConverter.convertToModelContent(
    ...
    public Model importSingleModel(Project project, ModelType modelType, FileContent fileContent) {
        Model model = importModel(project, modelType, fileContent);
        model = updateModelContent(model, fileContent);
        cleanModelIdList();
        return model;
    }
    ...
    public Model importModel(Project project, ModelType modelType, FileContent fileContent) {
        logger.debug(
                MessageFormat.format(
                        ...
                        modelType, fileContent.getFilename(), fileContent));
    ...
    @Override
    public Model importModelFromContent(
            Project project, ModelType modelType, FileContent fileContent) {
        Model model = null;
        if (modelTypeService.isJson(modelType)
                || ContentTypeUtils.isJsonContentType(fileContent.getContentType())) {
        ...
        if (model.getId() == null
                && (modelTypeService.isJson(modelType)
                        == ContentTypeUtils.isJsonContentType(fileContent.getContentType()))) {
    ...
    @Override
    public <T extends Task> List<T> getTasksBy(
            Project project, ModelType processModelType, @NonNull Class<T> clazz) {
        Assert.notNull(clazz, "Class task type it must not be null");
        return getProcessesBy(project, processModelType).stream()
    ...
    public List<Process> getProcessesBy(Project project, ModelType type) {
        return getModels(project, type, Pageable.unpaged()).stream()
                        ...
                        .orElseThrow(
                                () ->
                                        new ImportModelException(
                                                "Cannot convert json file content to model: "
                                                        + fileContent));
        model.setName(
                removeEnd(
                        removeExtension(fileContent.getFilename(), JSON),
                                ...
                                new ImportModelException(
                                        MessageFormat.format(
                                                "Unexpected extension was found for file to import"
                                                        + " model of type {0}: {1}",
                                                modelType.getName(), fileContent.getFilename())));
    }
    ...
    public void validateModelContent(Model model, ValidationContext validationContext) {
        validateModelContent(
                model.getType(), modelRepository.getModelContent(model), validationContext);
    }
    ...
    private ValidationContext getValidationContext(
            Model model, FileContent fileContent, @Nullable Project project) {
        if (!modelTypeService.isJson(findModelType(model))
                && fileContent.getContentType().equals(CONTENT_TYPE_JSON)) {
    ...
    @Override
    public void validateModelContent(
            Model model, FileContent fileContent, ValidationContext validationContext) {
        validateModelContent(model.getType(), fileContent.getFileContent(), validationContext);
    }
    ...
    @Override
    public void validateModelContent(
            Model model, FileContent fileContent, Project project, boolean validateUsage) {
        if (validateUsage) {
    ...
    private void validateModelContentAndUsage(
            Model model, byte[] modelContent, ValidationContext validationContext) {
        emptyIfNull(modelContentService.findModelValidators(model.getType())).stream()
                .forEach(
                        modelValidator ->
                                modelValidator.validateModelContent(
                                        model, modelContent, validationContext, true));
    }

    private void validateModelContent(
            String modelType, byte[] modelContent, ValidationContext validationContext) {
        emptyIfNull(modelContentService.findModelValidators(modelType)).stream()
                .forEach(
                        modelValidator ->
                                modelValidator.validateModelContent(
                                        modelContent, validationContext));
    }
    ...
    public void validateModelExtensions(Model model, FileContent fileContent) {
        ValidationContext validationContext =
                !modelTypeService.isJson(findModelType(model))
                        ? EMPTY_CONTEXT
    ...
    public void validateModelExtensions(Model model, FileContent fileContent, Project project) {
        validateModelExtensions(
                model.getType(), fileContent.getFileContent(), createValidationContext(project));
    }

    @Override
    public Page<Model> getGlobalModels(
            ModelType modelType, boolean includeOrphans, Pageable pageable) {
        return modelRepository.getGlobalModels(modelType, includeOrphans, pageable);
    }

    private void validateModelExtensions(
            String modelType, byte[] modelContent, ValidationContext validationContext) {
        emptyIfNull(modelExtensionsService.findExtensionsValidators(modelType)).stream()
                .forEach(
                        modelValidator ->
                                modelValidator.validateModelExtensions(
                                        modelContent, validationContext));
    }
    ...
    private ModelType findModelType(Model model) {
        return Optional.ofNullable(model.getType())
                ...
                .orElseThrow(
                        () ->
                                new UnknownModelTypeException(
                                        "Unknown model type: " + model.getType()));
    }

    @Override
    public List<ModelUpdateListener> findModelUpdateListeners(String modelType) {
        return (List<ModelUpdateListener>)
                emptyIfNull(modelUpdateListenersMapByModelType.get(modelType));
    }
}
