...
public class ProcessModelContentConverter
    ...
    @Override
    public Optional<BpmnProcessModelContent> convertToModelContent(
        byte[] bytes
    ) {
        ...
        try {
            return Optional
                .ofNullable(convertToBpmnModel(bytes))
    ...
    public byte[] convertToBytes(
        BpmnProcessModelContent bpmnProcessModelContent
    ) {
        return bpmnConverter.convertToXML(
            bpmnProcessModelContent.getBpmnModel()
        );
    }
    ...
    public BpmnModel convertToBpmnModel(byte[] modelContent)
        ...
        try (
            InputStreamReader reader = new InputStreamReader(
                new ByteArrayInputStream(modelContent)
            )
        ) {
            XMLStreamReader xmlReader = createSafeXmlInputFactory()
    ...
    public FileContent overrideModelId(
        ...
        if (processModelContent.isPresent()) {
            ...
            ReferenceIdOverrider referenceIdOverrider = new ReferenceIdOverrider(
                modelIdentifiers
            );
            this.overrideAllProcessDefinition(
                    modelContent,
    ...
    private void overrideAllIdReferences(
        ...
        process
            .getFlowElements()
            .forEach(element -> element.accept(referenceIdOverrider));
    }
}
