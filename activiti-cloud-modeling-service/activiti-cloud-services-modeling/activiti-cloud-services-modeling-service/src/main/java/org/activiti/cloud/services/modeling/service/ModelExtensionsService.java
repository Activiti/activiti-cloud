...
public class ModelExtensionsService {
    ...
    private List<ModelExtensionsValidator> getExtensionValidators(
        ...
        if (
            modelType
                .getName()
                .equals(
                    org.activiti.cloud.modeling.api.ProcessModelType.PROCESS
                )
        ) {
            return this.modelExtensionsValidatorsMapByModelType.get(
                    modelType.getName()
                );
        }

        if (
            this.modelExtensionsValidatorsMapByModelType.containsKey(
                    modelType.getName()
                )
        ) {
            return Stream
                .concat(
                    this.modelExtensionsValidatorsMapByModelType.get(
                            modelType.getName()
                        )
                        .stream(),
    ...
    public List<ModelExtensionsValidator> findExtensionsValidators(
        String modelType
    ) {
