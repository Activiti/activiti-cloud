...
public class BpmnModelCallActivityValidator implements BpmnCommonModelValidator {
    ...
    private final String INVALID_CALL_ACTIVITY_REFERENCE_DESCRIPTION =
            "Call activity '%s' with call element '%s' found in process '%s' references a process"
                    + " id that does not exist in the current project.";
    ...
    private final String INVALID_CALL_ACTIVITY_REFERENCE_NAME =
            "Invalid call activity reference validator.";
    private final String NO_REFERENCE_FOR_CALL_ACTIVITY_DESCRIPTION =
            "No call element found for call activity '%s' found in process '%s'. Call activity must"
                    + " have a call element that reference a process id present in the current"
                    + " project.";
    ...
    public Stream<ModelValidationError> validate(
        ...
        Set<String> availableProcessesIds =
                validationContext.getAvailableModels(processModelType).stream()
    ...
    private Stream<ModelValidationError> validateCallActivities(
            Set<String> availableProcessesIds, BpmnModel bpmnModel) {
        return processModelContentConverter
                .convertToModelContent(bpmnModel)
                .map(
                        converter ->
                                this.evaluateProcessCallActivity(
                                        converter, availableProcessesIds, bpmnModel))
                .orElse(Stream.empty());
    }
    ...
    private Stream<ModelValidationError> evaluateProcessCallActivity(
        ...
        return availableActivities.stream()
                .map(
                        activity ->
                                validateCallActivity(
                                        availableProcessesIds,
    ...
    private Optional<ModelValidationError> validateCallActivity(
            Set<String> availableProcessesIds, String mainProcess, CallActivity callActivity) {
        ...
        if (isEmpty(calledElement)) {
            return Optional.of(
                    new ModelValidationError(
                            format(
                                    NO_REFERENCE_FOR_CALL_ACTIVITY_PROBLEM,
                                    callActivity.getId(),
                                    mainProcess),
                            format(
                                    NO_REFERENCE_FOR_CALL_ACTIVITY_DESCRIPTION,
                                    callActivity.getId(),
                                    mainProcess),
