...
public abstract class ExtensionsJsonSchemaValidator extends JsonSchemaModelValidator
        implements ModelExtensionsValidator {

    @Autowired private JsonConverter<Model> extensionsConverter;
    ...
    public void validate(byte[] bytes, ValidationContext validationContext) {
        ...
        if (!validationContext.isEmpty()) {
            validateExtensionstInContext(bytes, validationContext);
        }
    }

    private void validateExtensionstInContext(byte[] bytes, ValidationContext validationContext) {
        List<ModelValidationError> validationExceptions =
                getValidationErrors(convertBytesToModel(bytes), validationContext);
    ...
    protected abstract List<ModelValidationError> getValidationErrors(
    ...
    protected Model convertBytesToModel(byte[] bytes) {
        try {
            ...
            throw new SyntacticModelValidationException(
                    "Cannot convert json extensions to a model", ex);
        }
    }
}
