...
public class ModelContentService {
    ...
    public ModelContentService(
        ...
        this.modelContentValidatorsMapByModelType =
            modelValidators
                .stream()
                .collect(
                    Collectors.groupingBy(validator ->
                        validator.getHandledModelType().getName()
                    )
                );
        ...
        this.modelContentConvertersMapByModelType =
            modelConverters
                ...
                .collect(
                    Collectors.toMap(
                        converter -> converter.getHandledModelType().getName(),
                        Function.identity()
                    )
                );
        this.contentUpdateListenersMapByModelType =
            contentUpdateListeners
                .stream()
                .collect(
                    Collectors.groupingBy(contentUpdateListener ->
                        contentUpdateListener.getHandledModelType().getName()
                    )
                );
    }
    ...
    public Optional<ModelContentConverter<? extends ModelContent>> findModelContentConverter(
        String modelType
    ) {
        return Optional.ofNullable(
            modelContentConvertersMapByModelType.get(modelType)
        );
    }

    public List<ContentUpdateListener> findContentUploadListeners(
        String modelType
    ) {
