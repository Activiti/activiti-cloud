...
public class JsonSchemaFlattenerTest {
    ...
    public void should_flattenProcessExtensionSchema() throws IOException {
        JSONObject schema = getSchemaFromResource(
            "/schema/process-extensions-schema.json"
        );
    ...
    @Test
    public void should_flattenSchemaShouldAddDefiniitons_when_noDefinitionsPresent()
        throws IOException {
        JSONObject schema = getTestJSONObjectWithoutDefinitions(
            "classpath://schema/model-extensions-schema.json"
        );
        assertThat(schema).isNotNull();
    ...
    @Test
    public void should_flattenSchemaShouldAddSection_when_definitionsArePresent()
        throws IOException {
        JSONObject schema = getTestJSONObjectWithDefinitions(
            "classpath://schema/model-extensions-schema.json"
        );
        assertThat(schema).isNotNull();

        String sectionName = flattener.getSectionNameFromFileName(
            "schema/model-extensions-schema.json"
        );
    ...
    private JSONObject getSchemaFromResource(String schemaFileName)
        throws IOException {
