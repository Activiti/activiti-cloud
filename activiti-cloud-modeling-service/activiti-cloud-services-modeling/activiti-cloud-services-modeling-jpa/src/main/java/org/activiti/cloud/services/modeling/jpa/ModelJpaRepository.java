...
public interface ModelJpaRepository extends JpaRepository<ModelEntity, String> {
    ...
    @Query(
            "SELECT m FROM Model m LEFT JOIN m.projects p WHERE p.id=:projectId AND"
    ...
    @Query(
            "SELECT m FROM Model m LEFT JOIN m.projects p WHERE p.id=:projectId AND"
                    + " m.name=:modelName AND m.type=:modelTypeFilter")
    ...
    @Query(
            "SELECT m FROM Model m WHERE m.name=:modelName AND m.scope=:scope AND"
                    + " m.type=:modelTypeFilter")
    ...
    Page<ModelEntity> findAllByScopeAndTypeEquals(
            ModelScope scope, String modelTypeFilter, Pageable pageable);

    @Query(
            "SELECT m FROM Model m LEFT JOIN m.projects p WHERE m.type=:modelTypeFilter AND"
                    + " (m.scope=:scope OR p.id IS NULL)")
