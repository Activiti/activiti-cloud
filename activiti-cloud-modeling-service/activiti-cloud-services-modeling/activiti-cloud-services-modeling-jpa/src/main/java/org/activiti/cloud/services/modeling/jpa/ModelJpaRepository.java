...
public interface ModelJpaRepository extends JpaRepository<ModelEntity, String> {
    @Query(
        "SELECT m FROM Model m LEFT JOIN m.projects p WHERE p.id=:projectId AND m.type=:modelTypeFilter"
    )
    ...
    @Query(
        "SELECT m FROM Model m WHERE m.name=:modelName AND m.scope=:scope AND m.type=:modelTypeFilter"
    )
