...
public class VersionedRepositoryMetadata extends DefaultRepositoryMetadata {
    ...
    public VersionedRepositoryMetadata(Class<?> repositoryInterface) {
        ...
        Class<?> type =
                resolveTypeParameter(
                        ...
                        () ->
                                String.format(
                                        "Could not resolve version entity type of %s",
                                        repositoryInterface));
        ...
        checkTypeParameter(
                ...
                () ->
                        String.format(
                                "The specified version entity type %s is not subtype of"
                                        + " VersionedEntity for repository %s",
                                getDomainType(), repositoryInterface));
        checkTypeParameter(
                type,
                VersionEntity.class,
                () ->
                        String.format(
                                "The specified version entity type %s is not subtype of"
                                        + " VersionEntity for repository %s",
    ...
    private static Class<?> resolveTypeParameter(
            Class<?> repositoryInterface, int index, Supplier<String> exceptionMessage) {

        List<TypeInformation<?>> arguments =
                ClassTypeInformation.from(repositoryInterface)
    ...
    private void checkTypeParameter(
            Class<?> typeToCheck, Class<?> expectedType, Supplier<String> exceptionMessage) {
