...
3. **Keep API calls in service/endpoint classes** — query/audit use hxp layout: one facade per service (`QueryService`, `AuditService`) with user + admin endpoint fields; endpoints use `admin: boolean` where paths differ. Specs call `service.<endpoint>.<method>()` for raw HTTP; use facade methods for `waitFor*`, `find*`, status checks, and health checks only.
