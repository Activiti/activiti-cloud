...
public class ActivitiAuditProducerEnvironmentPostProcessor implements EnvironmentPostProcessor {
    ...
    private static final Logger logger =
            LoggerFactory.getLogger(ActivitiAuditProducerEnvironmentPostProcessor.class);

    @Override
    public void postProcessEnvironment(
            ConfigurableEnvironment environment, SpringApplication application) {
        Optional<Boolean> isPartitioned =
                Optional.ofNullable(
                        environment.getProperty(
                                ACTIVITI_CLOUD_MESSAGING_PARTITIONED, Boolean.class));

        logger.warn("Configuring " + ACTIVITI_CLOUD_MESSAGING_PARTITIONED + "={}", isPartitioned);

        Integer partitionCount =
                environment.getProperty(ACTIVITI_CLOUD_MESSAGING_PARTITION_COUNT, Integer.class);
        ...
        isPartitioned
                ...
                .ifPresent(
                        value -> {
                            ...
                            properties.put(
                                    "spring.cloud.stream.bindings.auditProducer.producer.partitionCount",
                                    partitionCount);
