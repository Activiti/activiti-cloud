...
public class MessageProducerCommandContextCloseListener implements CommandContextCloseListener {
    ...
    public void closed(CommandContext commandContext) {
        List<CloudRuntimeEvent<?, ?>> events =
                commandContext.getGenericAttribute(PROCESS_ENGINE_EVENTS);

        if (events != null && !events.isEmpty()) {
            ExecutionContext rootExecutionContext =
                    commandContext.getGenericAttribute(ROOT_EXECUTION_CONTEXT);
            ...
            // Inject message headers with null execution context as there may be events from
