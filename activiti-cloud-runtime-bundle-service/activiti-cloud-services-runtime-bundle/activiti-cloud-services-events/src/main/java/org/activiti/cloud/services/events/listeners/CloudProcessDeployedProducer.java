...
public class CloudProcessDeployedProducer {
    ...
    @EventListener
    public void sendProcessDeployedEvents(
        ProcessDeployedEvents processDeployedEvents
    ) {
    ...
    protected void sendCloudProcessDeployedEvent(
        ...
        CloudRuntimeEvent<?, ?>[] payload = cloudProcessDeployedEvents.toArray(
            new CloudRuntimeEvent<?, ?>[] {}
        );

        Message<?> message = runtimeBundleMessageBuilderFactory
            .create()
            .withPayload(payload)
            .build();
