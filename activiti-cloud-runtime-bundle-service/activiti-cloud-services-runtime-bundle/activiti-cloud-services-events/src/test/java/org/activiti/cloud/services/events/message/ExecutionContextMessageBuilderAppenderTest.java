...
public class ExecutionContextMessageBuilderAppenderTest {
    ...
    private static final String MOCK_SUPER_EXECUTION_ID =
    ...
    public void testApply() {
        // given
        MessageBuilder<CloudRuntimeEvent<?, ?>> request = MessageBuilder.withPayload(
            new IgnoredRuntimeEvent()
        );
        ...
        assertThat(message.getHeaders())
            .containsEntry(
                ExecutionContextMessageHeaders.ROOT_BUSINESS_KEY,
                MOCK_BUSINESS_KEY
            )
            .containsEntry(
                ExecutionContextMessageHeaders.ROOT_PROCESS_INSTANCE_ID,
                MOCK_PROCESS_INSTANCE_ID
            )
            .containsEntry(
                ExecutionContextMessageHeaders.ROOT_PROCESS_DEFINITION_ID,
                MOCK_PROCESS_DEFINITION_ID
            )
            .containsEntry(
                ExecutionContextMessageHeaders.ROOT_PROCESS_DEFINITION_KEY,
            ...
            .containsEntry(
                ExecutionContextMessageHeaders.ROOT_PROCESS_NAME,
                MOCK_PROCESS_NAME
            )
            .containsEntry(
            ...
            .containsEntry(
                ExecutionContextMessageHeaders.DEPLOYMENT_ID,
                MOCK_DEPLOYMENT_ID
            )
            .containsEntry(
                ExecutionContextMessageHeaders.DEPLOYMENT_NAME,
                MOCK_DEPLOYMENT_NAME
            )
            .containsEntry(
                ExecutionContextMessageHeaders.DEPLOYMENT_VERSION,
                MOCK_APP_VERSION
            );
    }
    ...
    private ExecutionContext mockExecutionContext() {
        ...
        when(processInstance.getSuperExecutionId())
            .thenReturn(MOCK_SUPER_EXECUTION_ID);
