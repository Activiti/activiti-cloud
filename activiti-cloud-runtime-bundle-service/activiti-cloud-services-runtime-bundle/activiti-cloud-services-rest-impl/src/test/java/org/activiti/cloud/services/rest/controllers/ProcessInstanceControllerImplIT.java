...
class ProcessInstanceControllerImplIT {
    ...
    void getProcessInstances() throws Exception {
        ...
        //when
        mockMvc
            .perform(get("/v1/process-instances?page=0&size=10").accept(MediaTypes.HAL_JSON_VALUE))
            //then
    ...
    void getProcessInstancesShouldUseAlfrescoGuidelineWhenMediaTypeIsApplicationJson() throws Exception {
        ...
        mockMvc
            .perform(get("/v1/process-instances?skipCount=10&maxItems=10").accept(APPLICATION_JSON))
            .andExpect(status().isOk());
    }
    ...
    void should_getProcessInstanceByIdReturnNotFound_when_notFoundExceptionIsThrownByController() throws Exception {
        ...
        mockMvc
            .perform(get("/v1/process-instances/{processInstanceId}", processInstanceId))
    ...
    void should_getProcessDiagramReturnNotFound_when_notFoundExceptionIsThrownByController() throws Exception {
        ...
        mockMvc
            .perform(
                get("/v1/process-instances/{processInstanceId}/model", processInstanceId).contentType("image/svg+xml")
            )
            .andExpect(status().isNotFound())
            .andExpect(jsonPath("entry.code", is(404)))
            .andExpect(jsonPath("entry.message", is("not found")));
    ...
    void subprocesses() throws Exception {
        ...
        mockMvc.perform(get("/v1/process-instances/{processInstanceId}/subprocesses", 1)).andExpect(status().isOk());
    }
    ...
    @Test
    void startCreatedProcessShouldReturnBadRequestWhenVariableValueExceedsConfiguredSize() throws Exception {
        variableProperties.setMaxValueSize(5);
