...
public class ProcessInstanceControllerImplIT {
    ...
    public void getProcessInstances() throws Exception {
        //given
        List<ProcessInstance> processInstanceList = Collections.singletonList(
            defaultProcessInstance()
        );
        Page<ProcessInstance> processInstancePage = new PageImpl<>(
            processInstanceList,
            processInstanceList.size()
        );

        when(processRuntime.processInstances(any()))
            .thenReturn(processInstancePage);

        //when
        mockMvc
            .perform(
                get("/v1/process-instances?page=0&size=10")
    ...
    public void startProcess() throws Exception {
        StartProcessPayload cmd = ProcessPayloadBuilder
            .start()
    ...
    public void createProcess() throws Exception {
        CreateProcessInstancePayload cmd = ProcessPayloadBuilder
            .create()
            .withProcessDefinitionId("1")
            .build();
        when(processRuntime.create(any(CreateProcessInstancePayload.class)))
            .thenReturn(defaultProcessInstance());
    ...
    public void should_startProcessReturnForbidden_when_activitiForbiddenExceptionIsThrownByTheController()
        ...
        willThrow(new ActivitiForbiddenException("Not permitted"))
            ...
            .start(any(StartProcessPayload.class));
    ...
    public void getProcessInstanceById() throws Exception {
        when(processRuntime.processInstance("1"))
            .thenReturn(defaultProcessInstance());

        mockMvc
            .perform(get("/v1/process-instances/{processInstanceId}", 1))
            .andExpect(status().isOk());
    }
    ...
    public void getProcessDiagram() throws Exception {
        ...
        when(processRuntime.processInstance(anyString()))
        ...
        ProcessInstanceMeta processInstanceMeta = mock(
            ProcessInstanceMeta.class
        );
        when(processRuntime.processInstanceMeta(any()))
            .thenReturn(processInstanceMeta);
        when(processInstanceMeta.getActiveActivitiesIds())
        ...
        mockMvc
            .perform(
                get("/v1/process-instances/{processInstanceId}/model", 1)
    ...
    @Test
    public void should_getProcessDiagramReturnNotFound_when_notFoundExceptionIsThrownByController()
        throws Exception {
        String processInstanceId = "nonExistentProcessInstanceId";
        willThrow(new NotFoundException("not found"))
            .given(processRuntime)
    ...
    @Test
    public void should_getProcessDiagram_when_NoInterchangeInfo()
        throws Exception {
        ...
        given(repositoryService.getBpmnModel(processDefinitionId))
            .willReturn(bpmnModel);
        ProcessInstanceMeta processInstanceMeta = mock(
            ProcessInstanceMeta.class
        );
        given(processRuntime.processInstanceMeta(processInstanceId))
            .willReturn(processInstanceMeta);
        given(processInstanceMeta.getActiveActivitiesIds())
            .willReturn(emptyList());

        willThrow(
            new ActivitiInterchangeInfoNotFoundException(
                "No interchange information found."
            )
        )
            .given(processDiagramGenerator)
            .generateDiagram(bpmnModel, emptyList(), emptyList(), emptyList());
        ...
        mockMvc
            ...
            .andExpect(
                jsonPath(
                    "entry.message",
                    is("No interchange information found.")
                )
            );
    }
    ...
    public void sendSignal() throws Exception {
        SignalPayload cmd = ProcessPayloadBuilder
            .signal()
            .withName("signalInstance")
            .build();
    ...
    public void suspend() throws Exception {
        ...
        when(processRuntime.suspend(any()))
            .thenReturn(defaultProcessInstance());
        mockMvc
            .perform(
                post("/v1/process-instances/{processInstanceId}/suspend", 1)
            )
            .andExpect(status().isOk());
    }
    ...
    public void resume() throws Exception {
        ...
        mockMvc
            .perform(
                post("/v1/process-instances/{processInstanceId}/resume", 1)
            )
            .andExpect(status().isOk());
    }
    ...
    public void deleteProcessInstance() throws Exception {
        ...
        mockMvc
            .perform(delete("/v1/process-instances/{processInstanceId}", 1))
            .andExpect(status().isOk());
    }
    ...
    public void subprocesses() throws Exception {
        ...
        List<ProcessInstance> processInstanceList = singletonList(
            defaultProcessInstance()
        );
        Page<ProcessInstance> processInstances = new PageImpl<>(
            processInstanceList,
            processInstanceList.size()
        );

        when(processRuntime.processInstances(any(), any()))
            .thenReturn(processInstances);

        mockMvc
            .perform(
                get("/v1/process-instances/{processInstanceId}/subprocesses", 1)
            )
            .andExpect(status().isOk());
    }
    ...
    public void startMessage() throws Exception {
        ...
        when(processRuntime.start(any(StartMessagePayload.class)))
            .thenReturn(defaultProcessInstance());
