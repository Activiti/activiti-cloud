...
public class TaskControllerImplIT {
    ...
    public void getTasksShouldUseAlfrescoGuidelineWhenMediaTypeIsApplicationJson()
        ...
        this.mockMvc.perform(
                get("/v1/tasks?skipCount=10&maxItems=10")
    ...
    public void getTaskById() throws Exception {
        ...
        this.mockMvc.perform(get("/v1/tasks/{taskId}", 1))
            .andExpect(status().isOk());
    }
    ...
    public void claimTask() throws Exception {
        ...
        this.mockMvc.perform(post("/v1/tasks/{taskId}/claim", 1))
            .andExpect(status().isOk());
    }

    @Test
    public void releaseTask() throws Exception {
        given(taskRuntime.release(any()))
            .willReturn(buildTask("my task", CREATED));

        this.mockMvc.perform(post("/v1/tasks/{taskId}/release", 1))
            .andExpect(status().isOk());
    }

    @Test
    public void completeTask() throws Exception {
        given(taskRuntime.complete(any()))
            .willReturn(buildDefaultAssignedTask());
        this.mockMvc.perform(post("/v1/tasks/{taskId}/complete", 1))
            .andExpect(status().isOk());
    }

    @Test
    public void saveTask() throws Exception {
        SaveTaskPayload saveTask = TaskPayloadBuilder
            .save()
            .withTaskId("1")
    ...
    public void deleteTask() throws Exception {
        ...
        this.mockMvc.perform(delete("/v1/tasks/{taskId}", 1))
            .andExpect(status().isOk());
    }

    @Test
    public void getTaskByIdTaskNotFound() throws Exception {
        when(taskRuntime.task("not-existent-task"))
            .thenThrow(new NotFoundException("Not found"));

        this.mockMvc.perform(get("/v1/tasks/{taskId}", "not-existent-task"))
            .andExpect(status().isNotFound());
    }

    @Test
    public void createNewStandaloneTask() throws Exception {
        TaskImpl task = buildStandAloneTask(
            "new-task",
    ...
    public void getSubtasks() throws Exception {
        final TaskImpl subtask1 = buildTask(
            "subtask-1",
            "subtask-1 description"
        );
        subtask1.setPriority(85);

        final TaskImpl subtask2 = buildTask(
            "subtask-2",
            "subtask-2 description"
        );
        when(taskPage.getContent())
            .thenReturn(Arrays.asList(subtask1, subtask2));
        when(taskRuntime.tasks(any(), any())).thenReturn(taskPage);

        this.mockMvc.perform(
                get("/v1/tasks/{taskId}/subtasks", "parentTaskId")
                    .contentType(MediaType.APPLICATION_JSON)
            )
            .andExpect(status().isOk());
    }
    ...
    public void assignTask() throws Exception {
        ...
        AssignTaskPayload assignTaskCmd = TaskPayloadBuilder
            .assign()
            .withTaskId("1")
