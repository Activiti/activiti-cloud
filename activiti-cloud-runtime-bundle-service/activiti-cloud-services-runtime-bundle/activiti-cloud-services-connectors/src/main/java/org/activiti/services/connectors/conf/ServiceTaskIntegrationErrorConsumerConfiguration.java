...
public class ServiceTaskIntegrationErrorConsumerConfiguration {

    private static final Logger LOGGER = LoggerFactory.getLogger(ServiceTaskIntegrationErrorConsumerConfiguration.class);

    @Bean
    @ConditionalOnMissingBean(name = "integrationErrorsConsumer")
    Consumer<IntegrationError> integrationErrorsConsumer(IntegrationContextService integrationContextService, RuntimeService runtimeService,
                ...
                if (executions.size() > 0) {
                    ...
                    if (CloudBpmnError.class.getName().equals(errorClassName)) {
                        if (execution.getActivityId().equals(clientId)) {
                            try {
                                triggerIntegrationContextError(integrationError, execution, managementService, processEngineEventsAggregator,
    ...
    private void triggerIntegrationContextError(IntegrationError integrationError, ExecutionEntity execution, ManagementService managementService,
            ProcessEngineEventsAggregator processEngineEventsAggregator, RuntimeBundleProperties runtimeBundleProperties) {
