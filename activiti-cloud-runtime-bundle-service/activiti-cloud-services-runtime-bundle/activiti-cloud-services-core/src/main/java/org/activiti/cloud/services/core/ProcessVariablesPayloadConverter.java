...
public class ProcessVariablesPayloadConverter {
    ...
    public ProcessVariablesPayloadConverter(
        ProcessVariableValueConverter variableValueConverter
    ) {
        Assert.notNull(
            variableValueConverter,
            "VariableValueConverter must not be null"
        );
    ...
    private Map.Entry<String, Object> parseValue(
        ...
        try {
            if (Map.class.isInstance(entryValue)) {
                ...
                if (
                    valuesMap.containsKey("type") &&
                    valuesMap.containsKey("value")
                ) {
                    ...
                    entryValue =
                        variableValueConverter.convert(
                            new ProcessVariableValue(type, value)
                        );
                }
            } else if (ProcessVariableValue.class.isInstance(entryValue)) {
                entryValue =
                    variableValueConverter.convert(
                        ProcessVariableValue.class.cast(entryValue)
                    );
            }
