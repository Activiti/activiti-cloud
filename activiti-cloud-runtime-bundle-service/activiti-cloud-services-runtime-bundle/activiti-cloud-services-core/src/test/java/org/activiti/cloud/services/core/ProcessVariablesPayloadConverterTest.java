...
public class ProcessVariablesPayloadConverterTest {
    ...
    public void testProcessVariablesPayloadConverter() {
        ...
        input.put(
            "nullValue",
            new ProcessVariableValue("String", null).toMap()
        );
        input.put(
            "stringValue",
            new ProcessVariableValue("string", "name").toMap()
        );
        input.put(
            "quoteValue",
            new ProcessVariableValue("string", "\"").toMap()
        );
        input.put("intValue", new ProcessVariableValue("int", "10").toMap());
        input.put("longValue", new ProcessVariableValue("long", "10").toMap());
        input.put(
            "booleanValue",
            new ProcessVariableValue("boolean", "true").toMap()
        );
        input.put(
            "doubleValue",
            new ProcessVariableValue("double", "10.00").toMap()
        );
        input.put(
            "localDateValue",
            new ProcessVariableValue("localdate", "2020-04-20").toMap()
        );
        input.put(
            "dateValue",
        ...
        input.put(
            "jsonNodeValue",
            new ProcessVariableValue("json", "{}").toMap()
        );
        input.put("jsonNodeValue2", new ProcessVariableValue("json", "{}"));

        // when
        StartProcessPayload result = subject.convert(
            ProcessPayloadBuilder.start().withVariables(input).build()
        );
        // then
