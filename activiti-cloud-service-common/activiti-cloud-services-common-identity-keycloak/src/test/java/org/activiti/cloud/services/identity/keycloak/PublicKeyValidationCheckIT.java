...
class PublicKeyValidationCheckIT {
    ...
    void should_tokenBeValid() {
        Jwt accessToken = Jwt.withTokenValue(securityContextTokenProvider.getCurrentToken().get())
            .header("a", "b")
            .claim("a", "b")
            .build();
