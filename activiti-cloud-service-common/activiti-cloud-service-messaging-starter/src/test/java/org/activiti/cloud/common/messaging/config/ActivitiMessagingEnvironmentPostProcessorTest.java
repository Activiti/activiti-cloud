...
public class ActivitiMessagingEnvironmentPostProcessorTest {
    ...
    public void should_setDefaultBinderToRabbit_when_brokerIsRabbitmq() {
        //given
        final MutablePropertySources propertySources = mock(
        ...
        final ArgumentCaptor<MapPropertySource> captor = ArgumentCaptor.forClass(
        ...
        verify(propertySources)
            .addAfter(
                eq(SYSTEM_ENVIRONMENT_PROPERTY_SOURCE_NAME),
                captor.capture()
            );
        assertThat(
            captor
                .getValue()
                .getProperty(SPRING_CLOUD_STREAM_DEFAULT_BINDER_KEY)
        )
            .isEqualTo("rabbit");
    }
    ...
    public void should_setDefaultBinderToKafkaAndDisableRabbitHelfCheck_when_brokerIsKafka() {
        ...
        assertThat(
            captor.getValue().getProperty(MANAGEMENT_HEALTH_RABBIT_ENABLED_KEY)
        )
            .isEqualTo(false);
    }
    ...
    private ConfigurableEnvironment buildEnvironment(
        ...
        final ConfigurableEnvironment environment = mock(
