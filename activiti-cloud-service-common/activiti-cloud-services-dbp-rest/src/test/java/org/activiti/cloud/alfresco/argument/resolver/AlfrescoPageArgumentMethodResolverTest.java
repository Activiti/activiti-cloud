...
public class AlfrescoPageArgumentMethodResolverTest {
    ...
    @Test
    public void supportsParameterShouldReturnTrueWhenItsAPageable()
        throws Exception {
        //given
        MethodParameter parameter = buildParameter(Pageable.class);

        //when
        boolean supportsParameter = alfrescoPageArgumentMethodResolver.supportsParameter(
            parameter
        );
    ...
    @Test
    public void supportsParameterShouldReturnFalseWhenItsNotAPageable()
        throws Exception {
        //given
    ...
    public void resolveArgumentShouldReturnAPageableBasedOnParsedSkipCountAndMaxItems()
        ...
        ModelAndViewContainer modelAndViewContainer = mock(
            ModelAndViewContainer.class
        );

        NativeWebRequest webRequest = mock(NativeWebRequest.class);
        given(webRequest.getParameterMap())
            .willReturn(
                Collections.singletonMap("skipCount", new String[] { "40" })
            );
    ...
    @Test
    public void resolveArgumentShouldReturnTheBasePageableWhenNeitherSkipCountOrMaxItemsIsSet()
        throws Exception {
        //given
        MethodParameter methodParameter = mock(MethodParameter.class);
        ModelAndViewContainer modelAndViewContainer = mock(
        ...
        given(pageParameterParser.parseParameters(webRequest))
            .willReturn(
                new AlfrescoQueryParameters(
                    new SkipCountParameter(false, 0),
                    new MaxItemsParameter(false, 100)
                )
            );
