...
public class TaskEventContainedBuilder {
    ...
    public Task aCreatedStandaloneAssignedTaskWithParent(String taskName, String username) {
        ...
        eventsAggregator.addEvents(
                new CloudTaskCreatedEventImpl(task), new CloudTaskAssignedEventImpl(task));
        return task;
    }
    ...
    public Task anAssignedTaskWithDueDate(
            String taskName, String username, ProcessInstance processInstance, Date dueDate) {
        TaskImpl task = buildTask(taskName, Task.TaskStatus.ASSIGNED, processInstance);
        task.setAssignee(username);
    ...
    public Task anAssignedTaskWithParent(
            String taskName, String username, ProcessInstance processInstance) {
        TaskImpl task = buildTask(taskName, Task.TaskStatus.ASSIGNED, processInstance);
        task.setAssignee(username);
    ...
    public Task aTaskWithUserCandidate(
            String taskName, String username, ProcessInstance processInstance) {
        TaskImpl task = buildTask(taskName, Task.TaskStatus.CREATED, processInstance);
        eventsAggregator.addEvents(
                new CloudTaskCreatedEventImpl(task),
                new CloudTaskCandidateUserAddedEventImpl(
                        new TaskCandidateUserImpl(username, task.getId())));
        return task;
    }

    public Task aTaskWithGroupCandidate(
            String taskName, String groupId, ProcessInstance processInstance) {
        TaskImpl task = buildTask(taskName, Task.TaskStatus.CREATED, processInstance);
        eventsAggregator.addEvents(
                new CloudTaskCreatedEventImpl(task),
                new CloudTaskCandidateGroupAddedEventImpl(
                        new TaskCandidateGroupImpl(groupId, task.getId())));
        return task;
    }
    ...
    public Task aTaskWithTwoGroupCandidates(
        ...
        eventsAggregator.addEvents(
                new CloudTaskCreatedEventImpl(task),
                new CloudTaskCandidateGroupAddedEventImpl(
                        new TaskCandidateGroupImpl(groupIdOne, task.getId())),
                new CloudTaskCandidateGroupAddedEventImpl(
                        new TaskCandidateGroupImpl(groupIdTwo, task.getId())));
        return task;
    }
    ...
    public Task aTaskWithPriority(String taskName, int priority, ProcessInstance processInstance) {
        ...
        eventsAggregator.addEvents(
                new CloudTaskCreatedEventImpl(task), new CloudTaskCompletedEventImpl(task));
        return task;
    }

    public static TaskImpl buildTask(
            String taskName, Task.TaskStatus status, ProcessInstance processInstance) {
    ...
    public Task aCompletedTaskWithCompletedBy(
            String taskName, ProcessInstance processInstance, String completedBy) {
    ...
    public Task aCompletedTaskWithCompletionDate(
            String taskName, ProcessInstance processInstance, Date completedDate) {
        Task task = buildTask(taskName, Task.TaskStatus.COMPLETED, processInstance);
