...
public class TaskControllerHelperTest {
    ...
    @InjectMocks private TaskControllerHelper taskControllerHelper;

    @Mock private TaskRepository taskRepository;

    @Mock private AlfrescoPagedModelAssembler<TaskEntity> pagedCollectionModelAssembler;

    @Mock private QueryDslPredicateAggregator predicateAggregator;

    @Mock private TaskRepresentationModelAssembler taskRepresentationModelAssembler;

    @Mock private PagedModel<EntityModel<QueryCloudTask>> cloudTaskPagedModel;
    ...
    public void findAll_should_useFindByVariableNameAndValue_when_variableSearchIsSet() {
        ...
        given(predicateAggregator.applyFilters(initialPredicate, filters))
                .willReturn(extendedPredicate);

        VariableSearch variableSearch =
                new VariableSearch("var", new VariableValue<>("any"), "string");
        PageRequest pageable = PageRequest.of(0, 10);
        PageImpl<TaskEntity> pageResult =
                new PageImpl<>(Collections.singletonList(new TaskEntity()));
        given(
        ...
        given(
                        pagedCollectionModelAssembler.toModel(
                                pageable, pageResult, taskRepresentationModelAssembler))
                .willReturn(cloudTaskPagedModel);

        // when
        PagedModel<EntityModel<QueryCloudTask>> resultPagedModel =
                taskControllerHelper.findAll(initialPredicate, variableSearch, pageable, filters);
