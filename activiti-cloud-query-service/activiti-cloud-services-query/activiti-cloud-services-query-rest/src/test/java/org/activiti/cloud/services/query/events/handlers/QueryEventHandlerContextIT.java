...
public class QueryEventHandlerContextIT {
    ...
    public void shouldHaveHandlersForAllSupportedEvents() {
        ...
        assertThat(
            handlers.get(TaskRuntimeEvent.TaskEvents.TASK_CREATED.name())
        )
            .isInstanceOf(TaskCreatedEventHandler.class);
        assertThat(
            handlers.get(TaskRuntimeEvent.TaskEvents.TASK_ASSIGNED.name())
        )
            .isInstanceOf(TaskAssignedEventHandler.class);
        assertThat(
            handlers.get(TaskRuntimeEvent.TaskEvents.TASK_SUSPENDED.name())
        )
            .isInstanceOf(TaskSuspendedEventHandler.class);
        assertThat(
            handlers.get(TaskRuntimeEvent.TaskEvents.TASK_ACTIVATED.name())
        )
            .isInstanceOf(TaskActivatedEventHandler.class);
        assertThat(
            handlers.get(TaskRuntimeEvent.TaskEvents.TASK_COMPLETED.name())
        )
            .isInstanceOf(TaskCompletedEventHandler.class);
        assertThat(
            handlers.get(TaskRuntimeEvent.TaskEvents.TASK_CANCELLED.name())
        )
            .isInstanceOf(TaskCancelledEventHandler.class);
        assertThat(
            handlers.get(TaskRuntimeEvent.TaskEvents.TASK_UPDATED.name())
        )
        ...
        assertThat(
            handlers.get(VariableEvent.VariableEvents.VARIABLE_CREATED.name())
        )
            .isInstanceOf(VariableCreatedEventHandler.class);
        assertThat(
            handlers.get(VariableEvent.VariableEvents.VARIABLE_UPDATED.name())
        )
            .isInstanceOf(VariableUpdatedEventHandler.class);
        assertThat(
            handlers.get(VariableEvent.VariableEvents.VARIABLE_DELETED.name())
        )
        ...
        assertThat(
            handlers.get(IntegrationEvents.INTEGRATION_RESULT_RECEIVED.name())
        )
            .isInstanceOf(IntegrationResultReceivedEventHandler.class);
        assertThat(
            handlers.get(IntegrationEvents.INTEGRATION_ERROR_RECEIVED.name())
        )
