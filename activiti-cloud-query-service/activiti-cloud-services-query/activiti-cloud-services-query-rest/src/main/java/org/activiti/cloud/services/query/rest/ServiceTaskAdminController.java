...
public class ServiceTaskAdminController {
    ...
    public EntityModel<CloudServiceTask> findById(
        @PathVariable String serviceTaskId
    ) {
        Predicate filter = QServiceTaskEntity.serviceTaskEntity.id.eq(
            serviceTaskId
        );
        ...
        ServiceTaskEntity entity = entityFinder.findOne(
            ...
            filter,
            "Unable to find service task entity for the given id:'" +
            serviceTaskId +
            "'"
        );
