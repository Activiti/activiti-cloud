...
public abstract class ProcessInstanceDiagramControllerBase {
    ...
    public String generateDiagram(String processInstanceId) {
        ...
        if (!bpmnModel.hasDiagramInterchangeInfo()) new BpmnAutoLayout(
            bpmnModel
        )
            .execute();

        List<String> highLightedActivities = resolveCompletedActivitiesIds(
            processInstanceId
        );
        List<String> highLightedFlows = resolveCompletedFlows(
            bpmnModel,
            processInstanceId
        );
        List<String> currentActivities = resolveStartedActivitiesIds(
            processInstanceId
        );
        List<String> erroredActivities = resolveErroredActivitiesIds(
            processInstanceId
        );
    ...
    protected List<String> resolveCompletedFlows(
        BpmnModel bpmnModel,
    ...
    protected List<String> resolveStartedActivitiesIds(
        String processInstanceId
    ) {
    ...
    protected List<String> resolveCompletedActivitiesIds(
        String processInstanceId
    ) {
        return bpmnActivityRepository
            .findByProcessInstanceIdAndStatus(
                processInstanceId,
                BPMNActivityStatus.COMPLETED
            )
            .stream()
            .map(BPMNActivityEntity::getElementId)
            .distinct()
    ...
    protected List<String> resolveErroredActivitiesIds(
        String processInstanceId
    ) {
        return bpmnActivityRepository
            .findByProcessInstanceIdAndStatus(
                processInstanceId,
                BPMNActivityStatus.ERROR
            )
            .stream()
            .map(BPMNActivityEntity::getElementId)
            .distinct()
