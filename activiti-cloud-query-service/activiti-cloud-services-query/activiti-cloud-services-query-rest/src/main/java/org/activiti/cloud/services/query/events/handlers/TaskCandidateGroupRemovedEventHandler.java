...
public class TaskCandidateGroupRemovedEventHandler implements QueryEventHandler {
    ...
    private static Logger LOGGER =
            LoggerFactory.getLogger(TaskCandidateGroupRemovedEventHandler.class);
    private final EntityManager entityManager;
    private final EntityManagerFinder entityManagerFinder;

    public TaskCandidateGroupRemovedEventHandler(
    ...
    public void handle(CloudRuntimeEvent<?, ?> event) {
        ...
        Optional<TaskEntity> findResult =
                entityManagerFinder.findTaskWithCandidateGroups(taskCandidateGroup.getTaskId());

        // if a task was cancelled / completed do not handle this event
        if (findResult.isPresent() && !findResult.get().isInFinalState()) {
            // Persist into database
            try {
                TaskCandidateGroupId id =
                        new TaskCandidateGroupId(
                                taskCandidateGroup.getTaskId(), taskCandidateGroup.getGroupId());
                Optional.ofNullable(entityManager.find(TaskCandidateGroupEntity.class, id))
