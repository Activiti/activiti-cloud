...
public class TaskVariableDeletedEventHandler {
    ...
    private static Logger LOGGER = LoggerFactory.getLogger(
        TaskVariableDeletedEventHandler.class
    );

    private final EntityManager entityManager;
    private final EntityManagerFinder entityManagerFinder;

    public TaskVariableDeletedEventHandler(
    ...
    public void handle(CloudVariableDeletedEvent event) {
        ...
        Optional<TaskEntity> findResult = entityManagerFinder.findTaskWithVariables(
            taskId
        );
        ...
        if (findResult.isPresent() && !findResult.get().isInFinalState()) {
            try {
                ...
                LOGGER.debug(
                    "Error handling TaskVariableDeletedEvent[" + event + "]",
                    cause
                );
            }
        }
