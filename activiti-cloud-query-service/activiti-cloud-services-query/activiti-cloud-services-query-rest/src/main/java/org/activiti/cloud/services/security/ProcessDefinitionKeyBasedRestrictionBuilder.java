...
public class ProcessDefinitionKeyBasedRestrictionBuilder {
    ...
    private BooleanExpression equalsIgnoringCaseAndHyphen(
        StringPath propertyPath,
        String value
    ) {
        //expect to remove hyphens when passing in environment variables
        return Expressions
            .stringTemplate("replace({0},'-','')", propertyPath)
    ...
    private BooleanExpression buildServiceNameRestriction(
        ...
        BooleanExpression appNamePredicate = equalsIgnoringCaseAndHyphen(
            serviceNamePath,
            serviceName
        );
        appNamePredicate =
            appNamePredicate.or(
                equalsIgnoringCaseAndHyphen(serviceFullNamePath, serviceName)
            );
    ...
    public Predicate applyProcessDefinitionKeyFilter(
        ...
        Map<String, Set<String>> restrictions = securityPoliciesManager.getAllowedKeys(
    ...
    private BooleanExpression addProcessDefRestrictionToExpression(
        ...
        BooleanExpression appNamePredicate = buildServiceNameRestriction(
            appName,
