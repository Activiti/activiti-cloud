...
public class TaskCompletedEventHandler implements QueryEventHandler {
    ...
    public void handle(CloudRuntimeEvent<?, ?> event) {
        ...
        Optional<TaskEntity> findResult = Optional.ofNullable(
        ...
        queryTaskEntity.setLastModified(
            new Date(taskCompletedEvent.getTimestamp())
        );
        queryTaskEntity.setCompletedDate(
            new Date(taskCompletedEvent.getTimestamp())
        );
        queryTaskEntity.setCompletedBy(
            taskCompletedEvent.getEntity().getCompletedBy()
        );

        if (
            queryTaskEntity.getCompletedDate() != null &&
