...
public class TaskSpecification2 extends SpecificationSupport2<TaskEntity> {
    ...
    private TaskSpecification2(TaskSearchRequest taskSearchRequest, String userId, Collection<String> userGroups) {
     ...
     */
    public static TaskSpecification2 unrestricted(TaskSearchRequest taskSearchRequest) {
        return new TaskSpecification2(taskSearchRequest, null, null);
    }
     ...
     */
    public static TaskSpecification2 restricted(
        TaskSearchRequest taskSearchRequest,
        String userId,
        Collection<String> userGroups
    ) {
        return new TaskSpecification2(taskSearchRequest, userId, userGroups);
    }
    ...
    private void applyProcessVariableFilters(
        ...
        if (!CollectionUtils.isEmpty(taskSearchRequest.processVariableFilters())) {
            SetJoin<TaskEntity, ProcessVariableEntity> pvRoot = root.join(TaskEntity_.processVariables, JoinType.LEFT);

            Predicate[] filters = taskSearchRequest
                .processVariableFilters()
                .stream()
                .map(filter ->
                    criteriaBuilder.exists(
                        query
                            .subquery(ProcessVariableEntity.class)
                            .where(
                                criteriaBuilder.and(
            ...
            predicates.add(criteriaBuilder.and(filters));
        }
    }
        ...
        if (!CollectionUtils.isEmpty(taskSearchRequest.taskVariableFilters())) {
            ...
            Predicate[] variableValueFilters = taskSearchRequest
                ...
                .map(filter ->
                    criteriaBuilder.exists(
                        query
                            .subquery(TaskVariableEntity.class)
                            .where(
                                criteriaBuilder.and(
                                    criteriaBuilder.equal(
            ...
            predicates.add(criteriaBuilder.and(variableValueFilters));
        }
    }
