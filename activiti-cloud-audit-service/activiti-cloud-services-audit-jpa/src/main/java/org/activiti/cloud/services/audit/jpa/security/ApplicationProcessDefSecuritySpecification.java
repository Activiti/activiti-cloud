...
public class ApplicationProcessDefSecuritySpecification implements Specification<AuditEventEntity> {
    ...
    public ApplicationProcessDefSecuritySpecification(
            String serviceName, Set<String> processDefinitions) {
    ...
    public Predicate toPredicate(
        ...
        for (String processDef : processDefinitions) {
            // don't actually have definitionKey in the event but do have definitionId which should
            // contain it
            // format is e.g. SimpleProcess:version:id
            // and fact we're here means can't be wildcard
            Predicate processDefinitionMatchPredicate =
                    criteriaBuilder.like(root.get("processDefinitionId"), processDef + "%");
            ...
            Predicate appNameMatchPredicate =
                    criteriaBuilder.equal(
                            criteriaBuilder.upper(replacedServiceName),
                            serviceName.replace("-", "").toUpperCase());

            Predicate appRestrictionPredicate =
                    criteriaBuilder.and(processDefinitionMatchPredicate, appNameMatchPredicate);
