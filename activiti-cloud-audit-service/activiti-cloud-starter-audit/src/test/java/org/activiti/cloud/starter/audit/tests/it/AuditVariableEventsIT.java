...
class AuditVariableEventsIT {
    ...
    void should_supportAllCloudVariableEvents() {
        ...
        CloudVariableDeletedEventImpl variableDeletedEvent = new CloudVariableDeletedEventImpl(
            variableInstanceUpdate
        );
        ...
        await()
            .untilAsserted(() -> {
                ...
                Map<String, Object> createdFilter = Map.of(
                    "eventType",
                    VariableEvents.VARIABLE_CREATED.name()
                );
                ResponseEntity<PagedModel<CloudRuntimeEvent>> createdEventsPage = eventsRestTemplate.executeFind(
                    createdFilter
                );
                Collection<CloudRuntimeEvent> createdEvents = createdEventsPage
                    .getBody()
                    .getContent();
                ...
                Map<String, Object> updatedFilter = Map.of(
                    "eventType",
                    VariableEvents.VARIABLE_UPDATED.name()
                );
                ResponseEntity<PagedModel<CloudRuntimeEvent>> updatedEventsPage = eventsRestTemplate.executeFind(
                    updatedFilter
                );

                Collection<CloudRuntimeEvent> updatedEvents = updatedEventsPage
                    .getBody()
                    .getContent();
                ...
                Map<String, Object> deletedFilter = Map.of(
                    "eventType",
                    VariableEvents.VARIABLE_DELETED.name()
                );
                ResponseEntity<PagedModel<CloudRuntimeEvent>> deletedEventsPage = eventsRestTemplate.executeFind(
                    deletedFilter
                );
                Collection<CloudRuntimeEvent> deletedEvents = deletedEventsPage
                    .getBody()
                    .getContent();
