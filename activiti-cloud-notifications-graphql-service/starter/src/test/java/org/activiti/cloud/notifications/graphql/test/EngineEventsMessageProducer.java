...
import static org.activiti.cloud.notifications.graphql.test.EngineEventsMessageProducer.ENGINE_EVENTS_PRODUCER;
...
//TODO:  this class seems never used
@Component(ENGINE_EVENTS_PRODUCER)
public class EngineEventsMessageProducer implements SmartLifecycle, Supplier<Flux<Message<?>>> {
    ...
    public EngineEventsMessageProducer(StreamBridge streamBridge) {
    ...
    public void send(Message<?> message) {
        streamBridge.send(ENGINE_EVENTS_PRODUCER + "-out-0", message);
    }
    ...
    public Flux<Message<?>> get() {
        try {
            List<Map<String, Object>> events = new ObjectMapper().readValue(json, new TypeReference<>() {
            });
