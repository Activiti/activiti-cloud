...
public class ActivitiGraphQLControllerTest {
    ...
    private void ok(final GraphQLQueryRequest query)
        throws Exception, JsonProcessingException {
        ok(mapper.writeValueAsString(query));
    }

    private void ok(final String json)
        throws Exception, JsonProcessingException {
        perform(json)
        ...
        mockmvc
            .perform(
                get("/graphql")
                    .param(
                        "query",
                        "{Tasks(where: {name: {EQ: \"name\"}}){select{id}}}"
                    )
    ...
    public void testGraphqlQueryPostUnsupportedMediaType() throws Exception {
        mockmvc
            .perform(post("/graphql").contentType(MediaType.TEXT_HTML))
            .andExpect(status().is(415));
    ...
    public void testGraphqlArgumentsJson() throws Exception {
        String json =
            "{\"query\": \"{Tasks(where:{name:{EQ: \\\"title\\\"}}){select{ title genre }}\", \"arguments\": {\"title\": \"title\"}}";

        ok(json);

        verify(executor)
            .execute(
                "{Tasks(where:{name:{EQ: \"title\"}}){select{ title genre }}",
                null,
                null
            );
    }
    ...
    public void testGraphqlArgumentsEmptyString() throws Exception {
        String json =
            "{\"query\": \"{Tasks(where:{name:{EQ: \\\"title\\\"}}){select{id name}}\", \"arguments\": \"\"}";

        ok(json);

        verify(executor)
            .execute(
                "{Tasks(where:{name:{EQ: \"title\"}}){select{id name}}",
                null,
                null
            );
    }
    ...
    public void testGraphqlArgumentsNull() throws Exception {
        String json =
            "{\"query\": \"{Tasks(where:{name:{EQ: \\\"title\\\"}}){select{id name}}\", \"arguments\": null}";
    ...
    public void testGraphqlNoArguments() throws Exception {
        String json =
            "{\"query\": \"{Tasks(where:{name:{EQ: \\\"title\\\"}}){select{id name}}\"}";
    ...
    public void testGraphqlArgumentsParamsVariables() throws Exception {
        String query =
            "query TasksQuery($title: String!){Tasks(name: $title){id name}}";
        Map<String, Object> args = new HashMap<>();
        args.put("title", "value");
