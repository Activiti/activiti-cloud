...
public class ActivitiGraphQLControllerTest {
    ...
    @MockBean private GraphQLJpaExecutor executor;
    ...
    /**
     * Mock executor responses to be non-null in order for Mock MVC to produce correct output
     * content type in MockHttpResponse
     */
    @BeforeEach
    public void setUp() {
    ...
    public void testGraphqlQueryPostUnsupportedMediaType() throws Exception {
        mockmvc.perform(post("/graphql").contentType(MediaType.TEXT_HTML))
                .andExpect(status().is(415));
    ...
    public void testGraphqlArguments() throws Exception {
        GraphQLQueryRequest query =
                new GraphQLQueryRequest(
                        "query TasksQuery($title: String!){Tasks(where:{name: {EQ:"
    ...
    public void testGraphqlArgumentsJson() throws Exception {
        String json =
                "{\"query\": \"{Tasks(where:{name:{EQ: \\\"title\\\"}}){select{ title genre }}\","
                        + " \"arguments\": {\"title\": \"title\"}}";

        ok(json);

        verify(executor)
                .execute("{Tasks(where:{name:{EQ: \"title\"}}){select{ title genre }}", null, null);
    }
    ...
    public void testGraphqlArgumentsEmptyString() throws Exception {
        ...
        verify(executor)
                .execute("{Tasks(where:{name:{EQ: \"title\"}}){select{id name}}", null, null);
    }
