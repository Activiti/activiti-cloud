...
@SpringBootTest(
    properties = "spring.data.jpa.repositories.bootstrap-mode=default"
)
...
class ActivitiGraphQLSchemaAutoConfigurationTest {
    ...
    void correctlyDerivesSchemaFromGivenEntities() {
        ...
        //then
        assertThat(
            schema.getQueryType().getFieldDefinition("Task").getArgument("id")
        )
        ...
        //then
        assertThat(
            schema.getQueryType().getFieldDefinition("Task").getArguments()
        )
    ...
    void correctlyDerivesPageableSchemaFromGivenEntities() {
        ...
        assertThat(
            schema.getQueryType().getFieldDefinition("Tasks").getArguments()
        )
            .describedAs("Ensure query has correct number of arguments")
            .hasSize(2);
        ...
        assertThat(
            schema
                .getQueryType()
                .getFieldDefinition("ProcessVariables")
                .getArgument("page")
        )
            .describedAs("Ensure that collections can be queried on by page")
            .isNotNull();
