...
public class ObjectScalarTest {
    ...
    private Map<String, Object> variables = Collections.singletonMap(
        "varRef1",
        "value1"
    );
    ...
    public void testASTParsing() {
        // when
        assertThat(coercing.parseLiteral(mkStringValue("s"), variables))
            .isEqualTo("s");
        assertThat(coercing.parseLiteral(mkFloatValue("99.9"), variables))
            .isEqualTo(new BigDecimal("99.9"));
        assertThat(
            coercing.parseLiteral(
                mkIntValue(BigInteger.valueOf(666)),
                variables
            )
        )
            .isEqualTo(BigInteger.valueOf(666));
        assertThat(coercing.parseLiteral(mkBooleanValue(true), variables))
            .isEqualTo(true);
        assertThat(coercing.parseLiteral(mkNullValue(), variables))
            .isEqualTo(null);
        assertThat(coercing.parseLiteral(mkVarRef("varRef1"), variables))
            .isEqualTo("value1");
        assertThat(
            coercing.parseLiteral(
                mkArrayValue(
    ...
    public void testASTObjectParsing() {
        ...
        assertThat(coercing.parseLiteral(mkObjectValue(input), variables))
            .isEqualTo(expected);
    }
