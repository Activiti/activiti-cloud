...
public class GraphQLBrokerMessageHandler extends AbstractBrokerMessageHandler {
    ...
    public GraphQLBrokerMessageHandler(
        ...
        this.graphQLsubscriptionRegistry =
            new GraphQLBrokerSubscriptionRegistry();
    ...
    public GraphQLBrokerMessageHandler setBufferTimeSpanMs(
     ...
     * indicates whether this message handler is running. In other words the message handler must
     * first be running and then the {@code #isBrokerAvailable()} flag may still independently
     ...
     * {@code org.springframework.context.ApplicationListener&lt;BrokerAvailabilityEvent&gt;} to
     * receive notifications when broker becomes available and unavailable.
     */
    @Override
    ...
    protected void handleMessageInternal(Message<?> message) {
        ...
        if (
            ...
            message.getPayload() instanceof GraphQLMessage
        ) {
            logMessage(message);
            Message<GraphQLMessage> graphQLMessage = (Message<GraphQLMessage>) message;
            GraphQLMessageType graphQLMessagePayloadType = graphQLMessage
                .getPayload()
                .getType();
                ...
                case CONNECTION_INIT:
                    if (!isBrokerAvailable()) {
                        sendErrorMessageToClient(
                            ...
                            GraphQLMessageType.CONNECTION_ERROR,
                            message
                        );
                        return;
                    }

                    long[] clientHeartbeat = SimpMessageHeaderAccessor.getHeartbeat(
                        headers
                    );
    ...
    @Timed
    protected final void handleConnectionInitMessage(
        Message<GraphQLMessage> message
    ) {
        GraphQLMessage operationPayload = message.getPayload();

        GraphQLMessage connection_ack = new GraphQLMessage(
            operationPayload.getId(),
            GraphQLMessageType.CONNECTION_ACK
        );

        MessageHeaderAccessor headerAccessor = SimpMessageHeaderAccessor.getMutableAccessor(
            message
        );

        Message<?> responseMessage = MessageBuilder.createMessage(
            connection_ack,
    ...
    @Timed
    protected final void handleStartSubscription(
        Message<GraphQLMessage> message
    ) {
        ...
        ExecutionResult executionResult = graphQLSubscriptionExecutor.execute(
            parameters.getQuery(),
        ...
        if (executionResult.getErrors().isEmpty()) {
            if (executionResult.getData() == null) {
                ...
                Optional
                    .of(executionResult.<Publisher<ExecutionResult>>getData())
                    .ifPresent(data -> {
                        MessageChannel outboundChannel = getClientOutboundChannelForSession(
                            sessionId
                        );

                        GraphQLBrokerChannelSubscriber subscriber = new GraphQLBrokerChannelSubscriber(
                            message,
                            operationPayload.getId(),
                            outboundChannel,
                            bufferTimeSpanMs,
                        ...
                        graphQLsubscriptionRegistry.subscribe(
                            sessionId,
                            operationPayload.getId(),
                            subscriber,
                            () -> {
            ...
            GraphQLMessage startSubscriptionErrors = new GraphQLMessage(
                startSubscriptionMessage.getId(),
                GraphQLMessageType.ERROR,
                payload
            );

            Message<GraphQLMessage> errorMessage = MessageBuilder.createMessage(
                startSubscriptionErrors,
                headers
            );
    ...
    private void handleQueryOrMutation(
        String id,
        ...
        GraphQLMessage operationData = new GraphQLMessage(
            id,
            GraphQLMessageType.DATA,
            payload
        );

        Message<?> dataMessage = MessageBuilder.createMessage(
            operationData,
            headerAccessor.getMessageHeaders()
        );
        ...
        GraphQLMessage completeData = new GraphQLMessage(
            id,
            GraphQLMessageType.COMPLETE
        );

        Message<?> completeMessage = MessageBuilder.createMessage(
            completeData,
            headerAccessor.getMessageHeaders()
        );
    ...
    @Timed
    protected final void handleStopSubscription(
        Message<GraphQLMessage> message
    ) {
        ...
        graphQLsubscriptionRegistry.unsubscribe(
            sessionId,
            operationPayload.getId(),
            subscriber -> {
    ...
    @Timed
    protected final void handleConnectionTerminate(
        Message<GraphQLMessage> message
    ) {
    ...
    private void sendErrorMessageToClient(
        String errorText,
        ...
        Map<String, Object> payload = Collections.singletonMap(
            "errors",
            Collections.singletonList(errorText)
        );
        GraphQLMessage inputOperation = (GraphQLMessage) inputMessage.getPayload();
        GraphQLMessage connectionError = new GraphQLMessage(
            inputOperation.getId(),
            type,
            payload
        );
        MessageHeaderAccessor headerAccessor = SimpMessageHeaderAccessor.getMutableAccessor(
     ...
     */
    public GraphQLBrokerMessageHandler setTaskScheduler(
     ...
     * taskScheduler} in which case the default becomes "10000,10000" (in milliseconds).
     */
    public GraphQLBrokerMessageHandler setHeartbeatValue(long[] heartbeat) {
        if (
            ...
            throw new IllegalArgumentException(
                "Invalid heart-beat: " + Arrays.toString(heartbeat)
            );
        }
     ...
     */
    public void setHeaderInitializer(
    ...
    private void handleDisconnect(
        ...
        if (origMessage != null) {
            accessor.setHeader(
                SimpMessageHeaderAccessor.DISCONNECT_MESSAGE_HEADER,
                origMessage
            );
        }
        initHeaders(accessor);

        GraphQLMessage disconnect = new GraphQLMessage(
            null,
            GraphQLMessageType.CONNECTION_ERROR
        );

        Message<GraphQLMessage> message = MessageBuilder.createMessage(
            disconnect,
        ...
        public void run() {
            ...
            for (SessionInfo info : sessions.values()) {
                ...
                if (
                    ...
                    GraphQLMessage heartbeat = new GraphQLMessage(
                        null,
                        GraphQLMessageType.KA
                    );

                    getClientOutboundChannel()
                        .send(MessageBuilder.createMessage(heartbeat, headers));
                }
            }
        }
    }
