...
public class GraphQLBrokerMessageHandler extends AbstractBrokerMessageHandler {
        ...
        if (SimpMessageType.MESSAGE.equals(messageType)
            ...
            switch (graphQLMessagePayloadType) {
                case CONNECTION_INIT:
                    if (!isBrokerAvailable()) {
                        sendErrorMessageToClient(
                                BROKER_NOT_AVAILABLE, GraphQLMessageType.CONNECTION_ERROR, message);
                        return;
                    }
                ...
                case START:
                    ...
                    if (!isBrokerAvailable()) {
                        sendErrorMessageToClient(
                                BROKER_NOT_AVAILABLE, GraphQLMessageType.ERROR, message);
                        return;
                    }
    ...
    protected final void handleConnectionInitMessage(Message<GraphQLMessage> message) {
        ...
        GraphQLMessage connection_ack =
                new GraphQLMessage(operationPayload.getId(), GraphQLMessageType.CONNECTION_ACK);

        MessageHeaderAccessor headerAccessor =
                SimpMessageHeaderAccessor.getMutableAccessor(message);

        Message<?> responseMessage =
                MessageBuilder.createMessage(connection_ack, headerAccessor.getMessageHeaders());
    ...
    protected final void handleStartSubscription(Message<GraphQLMessage> message) {
        ...
        ExecutionResult executionResult =
                graphQLSubscriptionExecutor.execute(
                        parameters.getQuery(), parameters.getVariables());
        ...
        if (executionResult.getErrors().isEmpty()) {
            if (executionResult.getData() == null) {
                ...
                Optional.of(executionResult.<Publisher<ExecutionResult>>getData())
                        .ifPresent(
                                data -> {
                                    MessageChannel outboundChannel =
                                            getClientOutboundChannelForSession(sessionId);
            ...
            Map<String, Object> payload =
                    Collections.singletonMap("errors", executionResult.getErrors());

            GraphQLMessage startSubscriptionMessage = message.getPayload();
            GraphQLMessage startSubscriptionErrors =
                    new GraphQLMessage(
                            startSubscriptionMessage.getId(), GraphQLMessageType.ERROR, payload);

            Message<GraphQLMessage> errorMessage =
                    MessageBuilder.createMessage(startSubscriptionErrors, headers);
    ...
    private void handleQueryOrMutation(
            String id, ExecutionResult result, Message<GraphQLMessage> message) {
        ...
        Message<?> dataMessage =
                MessageBuilder.createMessage(operationData, headerAccessor.getMessageHeaders());
        ...
        Message<?> completeMessage =
                MessageBuilder.createMessage(completeData, headerAccessor.getMessageHeaders());
    ...
    private void sendErrorMessageToClient(
            String errorText, GraphQLMessageType type, Message<?> inputMessage) {
        Map<String, Object> payload =
                Collections.singletonMap("errors", Collections.singletonList(errorText));
        ...
        MessageHeaderAccessor headerAccessor =
                SimpMessageHeaderAccessor.getMutableAccessor(inputMessage);
        Message<GraphQLMessage> errorMessage =
                MessageBuilder.createMessage(connectionError, headerAccessor.getMessageHeaders());
    ...
    private void handleDisconnect(String sessionId, Principal user, Message<?> origMessage) {
        ...
        Message<GraphQLMessage> message =
                MessageBuilder.createMessage(disconnect, accessor.getMessageHeaders());
