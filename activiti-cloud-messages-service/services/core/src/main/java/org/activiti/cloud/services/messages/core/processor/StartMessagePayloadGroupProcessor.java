...
public class StartMessagePayloadGroupProcessor
    extends AbstractMessageGroupProcessorHandler {

    private final MessageGroupStore messageGroupStore;

    public StartMessagePayloadGroupProcessor(
    ...
    protected Collection<Message<?>> process(MessageGroup group) {
        Collection<Message<?>> result = group
            .getMessages()
            .stream()
            .filter(MESSAGE_SENT)
    ...
    protected Message<?> buildOutputMessage(Message<?> message) {
        StartMessagePayload startPayload = StartMessagePayloadTransformer.from(
            message
        );
    ...
    protected boolean canProcess(MessageGroup group) {
        ...
        return (
            messages.stream().anyMatch(START_MESSAGE_DEPLOYED) &&
