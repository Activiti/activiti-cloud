ACTIVITI_CLOUD_CONNECTORS_VERSION := 7.1.270
ACTIVITI_CLOUD_AUDIT_VERSION := 7.1.259
ACTIVITI_CLOUD_QUERY_VERSION := 7.1.270
ACTIVITI_CLOUD_RB_VERSION := 7.1.320
ACTIVITI_CLOUD_NOTIFICATIONS_VERSION := 7.1.292
ACTIVITI_CLOUD_MODELING :=7.1.548
...
updatebot/push-version:
	updatebot push-version --kind maven org.activiti.cloud.dependencies:activiti-cloud-dependencies $(ACTIVITI_CLOUD_VERSION) org.activiti.cloud.dependencies:activiti-cloud-modeling-dependencies $(MODELING_DEPENDENCIES_VERSION) --merge false
...
git-rev-list:
	...
	@echo "$(eval DIFF = $( shell git rev-list $(PREVIOUS_REV)..$(REV) --format=%B |grep -v ^commit))"
	$(eval DIFF = $( shell git rev-list $(PREVIOUS_REV)..$(REV) --format=%B |grep -v ^commit))
	@echo $(DIFF)
