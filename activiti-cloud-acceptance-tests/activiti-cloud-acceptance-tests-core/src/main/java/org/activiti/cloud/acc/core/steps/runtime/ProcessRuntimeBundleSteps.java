...
public class ProcessRuntimeBundleSteps {
    ...
    @Autowired private ProcessInstanceTasksApiClient processInstanceTasksApiClient;

    @Autowired private ProcessDefinitionsApiClient processDefinitionsApiClient;

    @Autowired private ProcessRuntimeDiagramService processRuntimeDiagramService;
    ...
    @Step
    public CloudProcessInstance startProcess(String process, boolean variables, String businessKey)
            throws IOException {
        ...
        if (variables) {
            ...
            payload.withVariable(
                    "test_bigdecimal_variable_name", wrap(BigDecimal.valueOf(1234567890L, 2)));
    ...
    public CloudProcessInstance startProcessWithVariables(
            String process, Map<String, Object> variables) {
        return startProcess(
                ProcessPayloadBuilder.start()
                        .withProcessDefinitionKey(process)
    ...
    public Collection<CloudProcessInstance> getAllProcessInstances() {
        return processInstanceApiClient.getProcessInstances(DEFAULT_PAGEABLE).getContent().stream()
