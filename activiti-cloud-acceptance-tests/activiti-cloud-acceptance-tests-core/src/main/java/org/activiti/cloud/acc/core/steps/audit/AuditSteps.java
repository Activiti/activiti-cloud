...
public class AuditSteps {
    ...
    @Step
    public Collection<CloudRuntimeEvent> getEventsByProcessInstanceId(
        String processInstanceId
    ) throws Exception {
        return auditService
            .getEvents("processInstanceId:" + processInstanceId)
            .getContent();
    }
    ...
    public void checkProcessInstanceEvent(
        ...
        await()
            ...
            .untilAsserted(() -> {
                ...
                assertThat(resultingEvent)
                    .isInstanceOf(CloudProcessRuntimeEvent.class);
        ...
        await()
            .untilAsserted(() -> {
                ...
                List<CloudRuntimeEvent> processInstanceTasks = events
                    .stream()
                    .filter(e ->
                        ((CloudTaskRuntimeEvent) e).getEntity()
                ...
                assertThat(resultingEvent).isNotNull();
                assertThat(resultingEvent)
                    .isInstanceOf(CloudTaskRuntimeEvent.class);
                assertThat(
                    ((CloudTaskRuntimeEvent) resultingEvent).getEntity().getId()
                )
                    .isEqualTo(taskId);
                assertThat(resultingEvent.getServiceName()).isNotEmpty();
                assertThat(resultingEvent.getServiceFullName()).isNotEmpty();
        ...
        await()
            .untilAsserted(() -> {
                ...
                List<CloudRuntimeEvent> processInstanceTasks = events
                    ...
                    .filter(e ->
                        variableName.equals(
                            ((CloudVariableEvent) e).getEntity().getName()
                        ) &&
                        processInstanceId.equals(
                            ((CloudVariableEvent) e).getEntity()
                                .getProcessInstanceId()
                        ) &&
                ...
                assertThat(resultingEvent).isNotNull();
                assertThat(resultingEvent)
                    .isInstanceOf(CloudVariableEvent.class);
                assertThat(
                    ((CloudVariableEvent) resultingEvent).getEntity().getName()
                )
            ...
            .untilAsserted(() -> {
                ...
                List<CloudRuntimeEvent> varEvents = events
                    ...
                    .filter(e ->
                        ...
                        taskId.equals(
                            ((CloudVariableEvent) e).getEntity().getTaskId()
                        )
                    )
                    .collect(Collectors.toList());
     ...
     */
    @Step
    public void checkTaskCreatedAndAssignedEventsWhenAlreadyAssigned(
        String taskId
    ) {
        final Collection<CloudRuntimeEvent> events = getEventsByEntityId(
            taskId
        );
        Condition<CloudRuntimeEvent> taskIsMatched = buildTaskMatchCondition(
            taskId
        );
    ...
    private Condition<CloudRuntimeEvent> buildTaskMatchCondition(
        String taskId
    ) {
        return new Condition<CloudRuntimeEvent>() {
            ...
            public boolean matches(CloudRuntimeEvent event) {
                return (
                    ...
                    taskId.equals(
                        ((CloudTaskRuntimeEvent) event).getEntity().getId()
                    )
                );
            }
        };
    ...
    public Collection<CloudRuntimeEvent> getEventsByProcessDefinitionKey(
        ...
        return auditService
            .getEvents("processDefinitionKey:" + processDefinitionKey)
            .getContent();
    }
        ...
        await()
            .untilAsserted(() ->
                assertThat(getEventsByEntityId(taskId))
                    ...
                    .contains(
                        tuple(taskId, TaskRuntimeEvent.TaskEvents.TASK_UPDATED)
                    )
            );
    ...
    public void checkProcessInstanceSubProcessEvents(String processInstanceId) {
        await()
            .untilAsserted(() -> {
                Collection<CloudRuntimeEvent> receivedEvents = getEventsByProcessInstanceId(
                    processInstanceId
                );
    ...
    @Step
    public Collection<CloudRuntimeEvent> getEventsByEventType(String eventType)
        throws Exception {
