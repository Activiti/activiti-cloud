...
public class AuditSteps {
    ...
    @Step
    public Collection<CloudRuntimeEvent> getEventsByProcessInstanceId(String processInstanceId)
            throws Exception {
    ...
    public Collection<CloudRuntimeEvent> getEventsByProcessInstanceIdAndEventType(
            String processInstanceId, String eventType) throws Exception {
        return auditService
    ...
    public void checkProcessInstanceEvent(
            String processInstanceId, ProcessRuntimeEvent.ProcessEvents eventType)
            throws Exception {
        checkProcessInstanceEvent(
                processInstanceId, eventType, TIMEOUT); // this is awaitility default
    }
    ...
    public void checkProcessInstanceEvent(
        ...
        Collection<CloudRuntimeEvent> events =
                getEventsByProcessInstanceIdAndEventType(processInstanceId, eventType.name());
    ...
    public void checkProcessInstanceTaskEvent(
            String processInstanceId, String taskId, TaskRuntimeEvent.TaskEvents eventType)
            throws Exception {
        ...
        await().untilAsserted(
                        () -> {
                            ...
                            assertThat(events)
                                    ...
                                    .containsOnly(eventType);
                            List<CloudRuntimeEvent> processInstanceTasks =
                                    events.stream()
                                            .filter(
                                                    e ->
                                                            ((CloudTaskRuntimeEvent) e)
                                                                    .getEntity()
                            ...
                            assertThat(((CloudTaskRuntimeEvent) resultingEvent).getEntity().getId())
                                    .isEqualTo(taskId);
                            assertThat(resultingEvent.getServiceName()).isNotEmpty();
                            assertThat(resultingEvent.getServiceFullName()).isNotEmpty();
                        });
    ...
    public void checkProcessInstanceVariableEvent(
            String processInstanceId, String variableName, VariableEvent.VariableEvents eventType)
            throws Exception {

        await().untilAsserted(
                        () -> {
                            Collection<CloudRuntimeEvent> events =
                                    getEventsByProcessInstanceIdAndEventType(
                                            processInstanceId, eventType.name());
                                                    ...
                                                    e ->
                                                            variableName.equals(
                                                                    ...
                                                                    && !((CloudVariableEvent) e)
                                                                            ...
                                                                            .isTaskVariable())
                                            .collect(Collectors.toList());
                            assertThat(processInstanceTasks)
                                    .hasSize(
                                            1); // could be more than one if there are multiple vars
                            CloudRuntimeEvent resultingEvent = processInstanceTasks.get(0);
                            assertThat(resultingEvent).isNotNull();
                            assertThat(resultingEvent).isInstanceOf(CloudVariableEvent.class);
                            assertThat(((CloudVariableEvent) resultingEvent).getEntity().getName())
                                    .isEqualTo(variableName);
                            assertThat(resultingEvent.getServiceName()).isNotEmpty();
                            assertThat(resultingEvent.getServiceFullName()).isNotEmpty();
                        });
    }
        ...
        await().untilAsserted(
                        () -> {
                            ...
                            assertThat(events)
                                    .extracting(e -> e.getEventType())
                                    .containsOnly(eventType);
                            List<CloudRuntimeEvent> varEvents =
                                    events.stream()
                                            .filter(
                                                    e ->
                                                            variableName.equals(
    ...
    /**
     * Check if a standalone task was created and assigned to it's creator
     *
    ...
    private Condition<CloudRuntimeEvent> buildTaskMatchCondition(String taskId) {
        return new Condition<CloudRuntimeEvent>() {
            ...
            public boolean matches(CloudRuntimeEvent event) {
                ...
                return event instanceof CloudTaskRuntimeEvent
    ...
    /**
     * Check if a task was created and assigned and completed
     *
     * @param taskId the id of the task (from rb)
     */
    @Step
                                ...
                                assertThat(getEventsByEntityId(taskId))
                                        ...
                                        .containsOnly(
                                                ...
                                                tuple(
                                                        taskId,
                                                        taskId,
                                                        // TODO change to DELETED status and
                                                        // TASK_DELETED event when RB is ready
        ...
        await().untilAsserted(
                        () ->
                                assertThat(events)
                                        ...
                                        .contains(
                                                tuple(subtaskId, subtaskId, parentTaskId),
                                                tuple(subtaskId, subtaskId, parentTaskId)));
    }
    ...
    @Step
    public Collection<CloudRuntimeEvent> getEventsByProcessAndEntityId(
            String processInstanceId, String entityId) {
        return auditService
                .getEvents("entityId:" + entityId + ",processInstanceId:" + processInstanceId)
                .getContent();
    }

    @Step
    public Collection<CloudRuntimeEvent> getEventsByProcessDefinitionKey(
                ...
                .filteredOn(
                        event ->
                                event.getServiceType()
                                        .equals(TaskRuntimeEvent.TaskEvents.TASK_UPDATED.name()))
    ...
    public void checkProcessInstanceSubProcessEvents(String processInstanceId) {
        ...
        await().untilAsserted(
                        () -> {
                            Collection<CloudRuntimeEvent> receivedEvents =
                                    getEventsByProcessInstanceId(processInstanceId);
    ...
    @Step
    public void checkProcessInstanceInclusiveGatewayEvents(
    ...
    public void checkProcessInstanceTimerScheduledEvents(
            String processInstanceId, String timerId, long timeoutSeconds) {

        await().atMost(timeoutSeconds, TimeUnit.SECONDS)
                .untilAsserted(
                        () -> {
                            Collection<CloudRuntimeEvent> receivedEvents =
                                    getEventsByProcessAndEntityId(processInstanceId, timerId);
                            assertThat(receivedEvents)
                                    .isNotEmpty()
    ...
    @Step
    public void checkProcessInstanceTimerFiredEvents(
            String processInstanceId, String timerId, long timeoutSeconds) {
    ...
    @Step
    public void checkProcessInstanceTimerExecutedEvents(
            String processInstanceId, String timerId, long timeoutSeconds) {
    ...
    @Step
    public void checkProcessInstanceTimerFailedEvents(
            String processInstanceId, String timerId, long timeoutSeconds) {
